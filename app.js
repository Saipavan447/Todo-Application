        ...
        case hasStatusProperty(request.query):
            if(status === "TO DO" || status === "IN PROGRESS"|| status === "DONE"){
                ...
                *
                ...
                response.send(data.map((eachItem)=> convertTodoDbObjectToResponseObject(eachItem)));
            }else{
                response.status(400);
...
app.get("/agenda/",async(request,response) => {
    ...
    if (isMatch(date,"yyyy-MM-dd")){
        ...
        response.send(responseResult.map((eachItem)=> convertTodoDbObjectToResponseObject(eachItem)));
    }else{
        response.status(400);
...
app.put("/todos/:todoId/",async(request,response) => {
    ...
    const perviousTodo = await database.get(perviousTodoQuery);
    const {
...
// scenario-1
        case requestBody.status !== undefined:
            if(status === "TO DO" || status = "IN PROGRESS" || status = "DONE"){
                updateTodoQuery=`
                UPDATE todo SET todo = '${todo}',priority='${priority}',status='${status}',category='${category}',due_date='${dueDate}' WHERE id = ${id};`;
